...
def serializers_measure() -> Results:
    results: Results = {}
    common_globs = {"serializer_case": serializer_case}
    stress_test = "for _ in range(100): serializer_case(serializer)"
    for k, serializer in {
        "json": JSONSerializer(),
        "pickle": UnsafePickleSerializer(),
    }.items():
        globs = common_globs | {"serializer": serializer}
        results[k] = timeit(stress_test, globals=globs, number=1000)
