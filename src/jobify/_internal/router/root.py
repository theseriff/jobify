...
class RootRoute(Route[ParamsT, ReturnT]):
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        self._chain_outer_middleware: OuterCallNext | None = None
...
class RootRouter(Router):
    ...
    async def _schedule(self, context: OuterContext) -> asyncio.Handle:
        if context.is_persist:
            dumper = context.jobify_config.dumper.dump
            params_type = context.func_spec.params_type
            args_dumped: dict[str, JSONCompat] = {
                name: dumper(arg, params_type[name])
                for name, arg in context.arguments.items()
            }
            msg = Message(
                job_id=context.job.id,
                name=context.runnable.name,
                arguments=args_dumped,
                trigger=context.trigger,
            )
            serializer = context.jobify_config.serializer.dumpb
            scheduled_job = ScheduledJob.create(
                name=context.runnable.name,
                job_id=context.job.id,
                message=serializer(dumper(msg, Message)),
                next_run_at=context.job.exec_at,
            )
            add_schedule_db = context.jobify_config.storage.add_schedule
            await add_schedule_db(scheduled_job)
